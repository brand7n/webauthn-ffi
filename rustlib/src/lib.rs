...
fn get_webauthn(rp_id: &str, rp_origin: &str) -> Result<&'static Webauthn, String> {
    ...
    Ok(WEBAUTHN
        .get()
        .expect("Webauthn instance should be initialized"))
}
...
fn handle_register_begin(v: &Value) -> Result<Value, String> {
    ...
    let req: RegisterBeginRequest = serde_json::from_value(v.clone())
        .map_err(|e| format!("Failed to parse register_begin request: {e}"))?;
...
mod tests {
    ...
    fn test_handle_register_begin_success() {
        ...
        assert!(
            result_value.get("challenge").is_some(),
            "Response should contain challenge"
        );
        assert!(
            result_value.get("registration").is_some(),
            "Response should contain registration"
        );
        assert!(
            result_value.get("uuid").is_some(),
        ...
        assert!(
            ...
            "publicKey should contain challenge"
        );
        assert!(
            public_key.get("rp").is_some(),
            "publicKey should contain rp"
        );
        assert!(
            public_key.get("user").is_some(),
            "publicKey should contain user"
        );
        assert!(
            public_key.get("pubKeyCredParams").is_some(),
            "publicKey should contain pubKeyCredParams"
        );
        assert!(
            public_key.get("timeout").is_some(),
            "publicKey should contain timeout"
        );
        assert!(
            public_key.get("attestation").is_some(),
            "publicKey should contain attestation"
        );
    ...
    fn test_handle_register_begin_missing_user_id() {
        ...
        assert!(result
            .unwrap_err()
            .contains("Failed to parse register_begin request"));
    }
    ...
    fn test_handle_register_begin_extra_fields() {
        ...
        assert!(
            result.is_ok(),
            "Should succeed with extra fields (they should be ignored)"
        );
    }
    ...
    fn test_handle_login_begin_success() {
        ...
        assert!(
            result_value.get("auth_state").is_some(),
            "Response should contain auth_state"
        );
        ...
        assert!(
            public_key.get("rpId").is_some(),
            "publicKey should contain rpId"
        );
        assert!(
            public_key.get("timeout").is_some(),
            "publicKey should contain timeout"
        ...
        assert!(
            public_key.get("userVerification").is_some(),
            "publicKey should contain userVerification"
        );
    }
    ...
    fn test_handle_login_begin_missing_passkeys() {
        ...
        assert!(
            result.is_err(),
            "Should fail when passkeys field is missing"
        );
        assert!(result
            .unwrap_err()
            .contains("Failed to parse login_begin request"));
    }
    ...
    fn test_handle_register_finish_missing_registration() {
        ...
        assert!(
            result.is_err(),
            "Should fail when registration field is missing"
        );
        assert!(result
            .unwrap_err()
            .contains("Failed to parse register_finish request"));
    }
    ...
    fn test_handle_register_finish_missing_client_data() {
        ...
        assert!(
            result.is_err(),
            "Should fail when client_data field is missing"
        );
        assert!(result
            .unwrap_err()
            .contains("Failed to parse register_finish request"));
    ...
    fn test_handle_login_finish_missing_auth_state() {
        ...
        assert!(
            result.is_err(),
            "Should fail when auth_state field is missing"
        );
        assert!(result
            .unwrap_err()
            .contains("Failed to parse login_finish request"));
    }
    ...
    fn test_rust_json_api_register_begin() {
        ...
        let result_ptr = unsafe { rust_json_api(input_cstr.as_ptr()) };
        ...
        assert!(
            parsed.get("challenge").is_some(),
            "Should contain challenge"
        );
        assert!(
            parsed.get("registration").is_some(),
    ...
    fn test_rust_json_api_login_begin() {
        ...
        assert!(
            parsed.get("auth_state").is_some(),
            "Should contain auth_state"
        );
    ...
    fn test_rust_json_api_null_input() {
        let result_ptr = unsafe { rust_json_api(std::ptr::null()) };

        assert!(
            !result_ptr.is_null(),
            "Should return error response for null input"
        );
    ...
    fn test_rust_json_api_missing_op() {
        ...
        assert!(
            !result_ptr.is_null(),
            "Should return error response for missing op"
        );
    ...
    fn test_integration_registration_flow() {
        ...
        assert!(
            register_begin_result.is_ok(),
            "Registration begin should succeed"
        );
        ...
        assert!(
            registration_state.is_object(),
            "Registration state should be an object"
        );
        ...
        if register_finish_result.is_err() {
            ...
            assert!(
                error.contains("Failed to finish registration")
    ...
    fn test_integration_authentication_flow() {
        ...
        if login_finish_result.is_err() {
            ...
            // Should fail with a meaningful error about invalid credential data
            assert!(
                error.contains("Failed to finish authentication")
    ...
    fn test_integration_c_api_flow() {
        ...
        let login_result_ptr = unsafe { rust_json_api(login_input_cstr.as_ptr()) };

        assert!(
            !login_result_ptr.is_null(),
        ...
        assert!(
            login_parsed.get("challenge").is_some(),
            "Should contain challenge"
        );
        assert!(
            login_parsed.get("auth_state").is_some(),
            "Should contain auth_state"
        );

        // Clean up
        unsafe { free_string(login_result_ptr) };
    }
}
...
fn handle_register_finish(v: &Value) -> Result<Value, String> {
    ...
    let req: RegisterFinishRequest = serde_json::from_value(v.clone())
        .map_err(|e| format!("Failed to parse register_finish request: {e}"))?;
...
fn start_registration(webauthn: &Webauthn, user_id: &str, user_name: &str) -> RegistrationOutput {
    ...
    let (challenge, registration) = webauthn
        .start_passkey_registration(uuid, user_id, user_name, None)
    ...
    let registration_json =
        serde_json::to_value(registration).expect("Failed to serialize registration state");
...
fn handle_login_begin(v: &Value) -> Result<Value, String> {
    ...
    let req: LoginBeginRequest = serde_json::from_value(v.clone())
        .map_err(|e| format!("Failed to parse login_begin request: {e}"))?;
...
fn handle_login_finish(v: &Value) -> Result<Value, String> {
    ...
    let req: LoginFinishRequest = serde_json::from_value(v.clone())
        .map_err(|e| format!("Failed to parse login_finish request: {e}"))?;
